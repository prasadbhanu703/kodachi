...
const ModalContainer = ({
    ...
    return (
        <div className={show ? "modal-container active" : "modal-container"}
            ...
            onClick={outsideDismiss ? closeFunction : () => { }}>
            <div
                className={show ? "card active" : "card inactive"}
                style={{ minHeight: maxHeightAllocated + 'px', maxWidth: maxWidthAllocated + 'px', overflowX: overflowX, borderRadius: borderRadius + 'px', backgroundColor: modalBg, ...cardStyle }}
                onClick={e => e.stopPropagation()}
            >
                {show && <div className="content">
                    {children}
                </div>}
                {showCloseBtn && <button type='button' className="close-cross" onClick={closeFunction}>
